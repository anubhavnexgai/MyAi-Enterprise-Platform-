...
# MyAi bridge patch (see vendor UPSTREAM.md): OpenRouter free-model resilience.
# Free models flap with upstream 429s; OpenRouter's `models` array makes it
# auto-fall-back to the next available model server-side (used by Deep Research,
# which fires many calls per run). Only applied for :free primaries, so a request
# can never be routed to a paid model.
...
def _apply_openrouter_free_fallback(payload: Dict, target_url: str, model: str) -> None:
    if "openrouter.ai" in (target_url or "") and str(model).endswith(":free"):
        # OpenRouter caps the `models` fallback array at 3 entries total.
        payload["models"] = ([model] + [m for m in _OPENROUTER_FREE_FALLBACKS if m != model])[:3]
...
def llm_call(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        _apply_openrouter_free_fallback(payload, target_url, model)
