    ...
    def start_research(
        ...
        async def _run():
            ...
            try:
                ...
                entry["status"] = "error"
                # Save whatever we have so the run never silently vanishes — even
                # if synthesis failed (e.g. a reasoning model returned empty), the
                # collected findings/partial report are persisted to the library.
                researcher = entry.get("researcher")
                ...
                try:
                    ...
                    logger.error(f"Failed to save errored research: {save_err}")
                try:
                    on_progress({"phase": "error", "message": str(e)[:200]})
                except Exception:
                    pass
