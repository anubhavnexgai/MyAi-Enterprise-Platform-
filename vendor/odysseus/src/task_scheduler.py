...
class TaskScheduler:
    ...
    async def _execute_llm_task(self, task, db) -> str:
        ...
        # Tasks store the BASE endpoint URL, but the LLM call POSTs the URL as-is.
        # Chat sessions store the full chat URL; tasks must convert too, else the
        # request hits /v1 (not /v1/chat/completions) and returns no completion.
        try:
            from src.endpoint_resolver import build_chat_url
            _eu = endpoint_url.rstrip("/")
            if not (_eu.endswith("/chat/completions") or _eu.endswith("/v1/messages") or _eu.endswith("/api/chat")):
                endpoint_url = build_chat_url(endpoint_url)
        except Exception:
            pass
