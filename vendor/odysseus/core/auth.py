...
# [MyAi bridge patch] honor ODYSSEUS_DATA_DIR so each tenant subprocess gets its
# OWN auth.json + sessions.json (the sessions path derives from this dir). Without
# this, every tenant would share one user/session store. See UPSTREAM.md.
DEFAULT_AUTH_PATH = os.path.join(
    os.environ.get("ODYSSEUS_DATA_DIR") or os.path.join(Path(__file__).parent.parent, "data"),
    "auth.json",
)
