...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if is_report:
            ...
            # [MyAi bridge patch] When ODYSSEUS_ALLOW_EMBED=1 the instance is being
            # embedded in MyAi via an iframe, so it must NOT forbid framing. We drop
            # X-Frame-Options and relax frame-ancestors below. See UPSTREAM.md.
            _embed = os.getenv("ODYSSEUS_ALLOW_EMBED") == "1"
            if not _embed:
            ...
            response.headers["Content-Security-Policy"] = (
                ...
                "connect-src 'self'; "
                "frame-src 'self'; "
                + ("frame-ancestors *" if _embed else "frame-ancestors 'none'")
            )
        return response
