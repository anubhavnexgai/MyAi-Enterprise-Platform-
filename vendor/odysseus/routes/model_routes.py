...
def setup_model_routes(model_discovery):
    ...
    def _fetch_models(owner: str = "", is_admin: bool = False):
        ...
        for ep in endpoints:
            ...
            # MyAi bridge patch (see UPSTREAM.md): surface admin-pinned IDs even
            # when the endpoint is unprobed (curated whitelist support)...
            _pinned = _normalize_model_ids(getattr(ep, "pinned_models", None))
            if _pinned:
                model_ids = _merge_model_ids(model_ids, _pinned)
            # ...and for OpenRouter, expose ONLY free models ($0; id ends ":free")
            # so the picker can NEVER select a paid model, no matter what a probe
            # or background refresh cached. Rule-based => future paid models stay
            # excluded automatically.
            if "openrouter.ai" in (base or "").lower():
                model_ids = [m for m in model_ids if str(m).endswith(":free")]
